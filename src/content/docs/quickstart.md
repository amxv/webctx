...
Search uses Brave, Tavily, and Exa. Public GitHub repository/blob/tree/Issue/PR-conversation/label/milestone reads work without a token, while optional `GH_TOKEN` or `GITHUB_TOKEN` enables authenticated/private GitHub reads and richer PR review-thread state. Firecrawl handles site mapping and `read-link` pages that reach the generic fallback.
