...
- **Last completed phase:** Phase 7 — `Releases, trees, statistics, and deployment fan-out hardening`.
- **Earliest incomplete phase:** Phase 8 — `Discussions and Gists bounded navigation plus exact comment selectors`.
- **Observable boundary:** release detail now previews oversized notes, fetches only the first 100-asset provider page and bounds the asset index while retaining exact download/canonical release URLs; one-level tree views keep sorted identity and README preview but bound indexed entries while separating local omission from GitHub's 1,000-entry Contents ceiling; contributor statistics sort by commit total with stable login ties and bound rendered rows while keeping provider-cache/202 truth; deployment-environment detail reads only the latest returned status for each of its bounded deployments and records when older status pages exist instead of expanding status history. Discussion/Gist conversation and multi-file fan-out is the next boundary.
