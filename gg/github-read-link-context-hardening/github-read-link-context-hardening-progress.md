...
- **Last completed phase:** Phase 5 — `Actions overview/run/job context safety and raw-log navigation`.
- **Earliest incomplete phase:** Phase 6 — `Commit and compare overview/raw split plus commit selectors`.
- **Observable boundary:** Actions root/workflow/run/job views now use the shared context budget: the root indexes only representative workflows/runs with explicit `/actions/workflows` and page navigation, run details keep complete latest-job metadata for failure ordering but fetch only one artifact page, and selected jobs preserve structured steps while replacing giant logs with adaptive failure-context or head/tail previews. Stable GitHub job-log API URLs expose explicit raw navigation without printing the one-minute signed redirect. Plain commit/compare output is now the next high-amplification boundary.
