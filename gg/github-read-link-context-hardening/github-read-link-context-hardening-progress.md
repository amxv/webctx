...
- **Last completed phase:** Phase 6 — `Commit and compare overview/raw split plus commit selectors`.
- **Earliest incomplete phase:** Phase 7 — `Releases, trees, statistics, and deployment fan-out hardening`.
- **Observable boundary:** plain commit roots now fetch only the first file/comment provider pages and render the full commit message plus bounded file/comment indexes with exact `#diff-*` / `#commitcomment-*` selectors and raw `.diff`/`.patch` links; exact diff selectors may traverse provider pages to return one selected file/hunk, and exact commit comments use the single-comment endpoint with commit ownership validation. Plain compares now stop after the first provider page and render bounded commit/file indexes while preserving GitHub's separate 300-file compare ceiling; raw compare `.diff`/`.patch` remain explicit complete representations. Release/tree/statistics/deployment fan-out is the next boundary.
